...
class oapost {
    ...
    public function __construct(&$db, $ip_id) {
        ...
        $this->ip_id = $ip_id;
        $this->fields = array('id', 'post_title', 'post_content', 'post_date', 'post_modified', 'user_group','post_ip', 'post_type', 'post_order', 'post_parent', 'post_user', 'post_password', 'post_name', 'post_url', 'post_status', 'post_meta');
    }

    /**
     * 权限查询
     * @since 1
     * @param int $power 用户权限
     */
     public function view_power($id){
         $sql = 'SELECT user_group as `power` FROM moyun_posts  WHERE id = ?';
         $st = $this->db->prepare($sql);
         $st->bindParam(1, $id, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT);
         $res = null;
         if ($st->execute() === true) {
    ...
    public function view_list($user = null, $title = null, $content = null, $status = 'public', $type = 'text', $page = 1, $max = 10, $sort = 7, $desc = true, $parent = null, $name = '', $pw = null) {
        ...
        $sql = 'SELECT `id`,`post_title`,`post_date`,`post_modified`,`user_group`,`post_ip`,`post_type`,`post_order`,`post_parent`,`post_user`,`post_password`,`post_name`,`post_url`,`post_status`,`post_meta` FROM `' . $this->table_name . '` WHERE ' . $sql_where . ' `post_type`=:type ORDER BY ' . $this->fields[$sort] . ' ' . $sql_desc . ' LIMIT ' . ($page - 1) * $max . ',' . $max;
     ...
     * @return int 0或记录ID
     */
    public function add($title, $content, $power, $type, $parent, $user, $pw, $name, $url, $status, $meta) {
        $return = 0;
        $sql = 'INSERT INTO `' . $this->table_name . '`(`post_title`,`post_content`,`post_date`,`user_group`,`post_ip`,`post_type`,`post_order`,`post_parent`,`post_user`,`post_password`,`post_name`,`post_url`,`post_status`,`post_meta`) VALUES(:title,:content,NOW(),:power,:ip,:type,0,:parent,:user,:pw,:name,:url,:status,:meta)';
        $sth = $this->db->prepare($sql);
        $sth->bindParam(':title', $title, PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT);
        $sth->bindParam(':content', $content, PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT);
        $sth->bindParam(':power', $power, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT);
